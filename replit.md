...
- June 30, 2025: PROFESSIONAL EMAIL DOCUMENTATION SYSTEM IMPLEMENTED - Created comprehensive email system for sending legal documents to clients via Gmail. Features include: EmailDocumentationService with multiple provider support (Supabase Email, Gmail SMTP, SendGrid), professional HTML email templates with Resolve AI branding, admin panel integration with "Email Docs" tab, case integration for auto-filling client details, bulk email functionality, API endpoints for single/bulk sending, document attachment support, custom messaging, and complete setup guide. Authentication required for admin access with proper error handling and delivery tracking.
- June 30, 2025: CONTACT SYSTEM LOGIN REQUIREMENT IMPLEMENTED - Updated contact form to require user authentication before access. Unauthenticated users now see "You need to log in to contact us" message with direct link to sign-in page. Maintains all existing functionality for logged-in users while restricting access appropriately. Works on both navigation contact button and landing page contact section.
- June 30, 2025: COMPLETE CONTACT SYSTEM WITH SUPABASE INTEGRATION & NOTIFICATIONS - Implemented comprehensive contact system with full Supabase database integration. Features include: contact form submissions saved to Supabase contact_submissions table, admin dashboard for viewing/responding to submissions, automatic user notifications when admin responds, email sending functionality via Supabase Auth, user endpoint to view their own contact submission responses, real-time status updates (unread/read/responded), and complete admin response workflow. Contact email set to hello@resolveai.com throughout application.
- June 30, 2025: GOOGLE CALENDAR COMING SOON OVERLAY IMPLEMENTED - Added professional "Coming Soon" overlay component for all Google Calendar integration features due to pending Google app verification. Created reusable ComingSoonOverlay component that preserves original calendar functionality while showing users clear messaging about verification process. Applied to dashboard calendar sections and entire calendar page while maintaining all original components for future activation.
